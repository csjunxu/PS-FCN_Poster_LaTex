...
\begin{poster}{
 ...
 {
    \begin{tabular}{r}
        \includegraphics[width=0.2\linewidth]{cropped-Logo_ECCV18_RGB_1200x280}
    \end{tabular}
 }
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\headerbox{\bf\color{blue} Problem, Motivation, and Contributions}{name=contribution,column=0,row=0,span=2}{
    \textbf{\color{blue}Problem:} Estimating the latent clean image from the input real-world noisy image.
...
\textbf{\color{blue}TWSC:} Given a color image patch $\mathbf{Y}=\mathbf{X}+\mathbf{N}\in{3p^2\times N}$ and $\mathbf{Y}=\mathbf{D}\mathbf{S}\mathbf{V}^{\top}$ is the SVD of $\mathbf{Y}$, where $\mathbf{S}$ is the singular value matrix of $\mathbf{Y}$. The TWSC model can be written as
\vspace{-4mm}
\begin{equation}
\vspace{-4mm}
\label{e4}
